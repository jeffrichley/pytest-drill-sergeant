...
"""
        ...
        with patch.object(collector, "_resolve_import_to_file") as mock_resolve:
            mock_resolve.side_effect = lambda name, files: (
                Path(f"/test/{name}.py") if name in ["os", "sys"] else None
            )
...
"""
        ...
        with patch.object(collector, "_resolve_import_to_file") as mock_resolve:
            mock_resolve.side_effect = lambda name, files: (
                Path(f"/test/{name}.py") if name in ["pathlib", "collections"] else None
            )
...
"""
        ...
        with patch.object(collector, "_resolve_import_to_file") as mock_resolve:
            mock_resolve.side_effect = lambda name, files: (
                Path(f"/test/{name}.py") if name in ["json", "pickle"] else None
            )
...
"""
        ...
        # Check that results are stored
        assert (
            len(collector._called_functions) == 5
        )  # func, callable_result, subscript_call, obj.method, assert
...
"""
        ...
        with patch.object(collector, "_resolve_import_to_file") as mock_resolve:
            mock_resolve.side_effect = lambda name, files: (
                Path(f"/test/{name}.py") if name in ["os", "pathlib"] else None
            )
        ...
        assert (
            len(collector._called_functions) >= 3
        )  # At least os.path.join, Path, assert
