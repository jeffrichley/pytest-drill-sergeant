...
class TestFileDiscoveryConfig:
    ...
    def test_config_creation(self, tmp_path):
        ...
        config = create_test_config(
            ...
            per_file_rule_overrides={
                "test_special.py": {"DS201": RuleConfig(enabled=False)}
            },
        )
        ...
        assert (
            len(discovery_config.include_patterns) == 2
        )  # test_patterns + include_patterns
        assert (
            len(discovery_config.exclude_patterns) == 2
    ...
    def test_get_ignored_rules(self, tmp_path):
        ...
        assert set(discovery_config.get_ignored_rules(legacy_file)) == {
            "DS201",
            "DS202",
        }
    ...
    def test_get_severity_override(self, tmp_path):
        ...
        assert (
            discovery_config.get_severity_override(critical_file) == SeverityLevel.ERROR
        )
        assert (
            discovery_config.get_severity_override(warning_file)
...
class TestFileDiscovery:
    ...
    def test_get_effective_severity(self, tmp_path):
        ...
        assert (
            discovery.get_effective_severity(
                critical_file, "DS201", SeverityLevel.WARNING
            )
            == SeverityLevel.ERROR
        )
        ...
        assert (
            discovery.get_effective_severity(
                special_file, "DS201", SeverityLevel.WARNING
            )
            == SeverityLevel.INFO
        )
        ...
        assert (
            discovery.get_effective_severity(
                normal_file, "DS201", SeverityLevel.WARNING
            )
            == SeverityLevel.WARNING
        )
