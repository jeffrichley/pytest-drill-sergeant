...
class TestFileWatcherInitialization:
    ...
    def test_file_watcher_init(self):
        ...
        assert (
            watcher.logger.name == "pytest_drill_sergeant.lsp.file_watcher.FileWatcher"
        )
    ...
    def test_file_watcher_init_with_logger(self):
        ...
        with patch(
            "pytest_drill_sergeant.lsp.file_watcher.logging.getLogger"
        ) as mock_get_logger:
            mock_logger = Mock()
            ...
            mock_get_logger.assert_called_once_with(
                "pytest_drill_sergeant.lsp.file_watcher.FileWatcher"
            )
...
class TestDocumentAnalysis:
    ...
    def test_analyze_document_if_needed_successful_analysis(self):
        ...
        with patch(
            ...
            with patch.object(watcher, "_should_analyze_document", return_value=True):
                ...
                mock_server.publish_diagnostics.assert_called_once_with(
                    mock_document.uri, mock_diagnostics
                )
    ...
    def test_analyze_document_if_needed_should_analyze_error(self):
        ...
        with patch.object(
            watcher, "_should_analyze_document", side_effect=Exception("Check failed")
        ):
            watcher._analyze_document_if_needed(mock_document)
...
class TestDocumentTypeDetection:
    ...
    def test_should_analyze_document_exception_handling(self):
        ...
        with patch(
            ...
            with patch(
                "pytest_drill_sergeant.lsp.file_watcher.Path",
...
class TestTestFileDetection:
    ...
    def test_is_test_file_case_sensitivity(self):
        ...
        assert (
            result is True
        )  # Actually matches because 'test_module' starts with 'test_'
...
class TestAnalyzingFilesTracking:
    ...
    def test_analyzing_files_tracking_error_cleanup(self):
        ...
        with patch.object(watcher, "_should_analyze_document", return_value=True):
            with patch.object(
                watcher.server, "analyze_document", side_effect=Exception("Error")
            ):
                watcher._analyze_document_if_needed(mock_document)
...
class TestIntegrationScenarios:
    ...
    def test_file_watching_with_different_file_types(self):
        ...
        test_cases = [
            ...
            (
                "file:///project/src/test_dir/module.py",
                False,
            ),  # Not a test file (test_dir != test)
