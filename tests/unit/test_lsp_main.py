...
class TestCreateServer:
    ...
    def test_create_server_returns_language_server(self):
        ...
        with patch("pytest_drill_sergeant.lsp.main.DrillSergeantLanguageServer") as mock_server_class:
            with patch("pytest_drill_sergeant.lsp.main.setup_file_watching") as mock_setup:
                mock_server = Mock()
    ...
    def test_create_server_handles_import_errors(self):
        ...
        with patch("pytest_drill_sergeant.lsp.main.DrillSergeantLanguageServer", side_effect=ImportError("Module not found")):
    ...
    def test_create_server_handles_setup_errors(self):
        ...
        with patch("pytest_drill_sergeant.lsp.main.DrillSergeantLanguageServer") as mock_server_class:
            with patch("pytest_drill_sergeant.lsp.main.setup_file_watching", side_effect=Exception("Setup failed")):
                mock_server = Mock()
                mock_server_class.return_value = mock_server
...
class TestMainAsync:
    ...
    async def test_main_sets_up_logging(self):
        ...
        with patch("pytest_drill_sergeant.lsp.main.logging.basicConfig") as mock_basic_config:
            with patch("pytest_drill_sergeant.lsp.main.logger") as mock_logger:
                with patch("pytest_drill_sergeant.lsp.main.create_server") as mock_create:
                    mock_server = Mock()
        ...
        with patch("pytest_drill_sergeant.lsp.main.logging.basicConfig"):
            with patch("pytest_drill_sergeant.lsp.main.logger") as mock_logger:
                with patch("pytest_drill_sergeant.lsp.main.create_server") as mock_create:
                    ...
                    mock_logger.info.assert_called_with("Starting drill sergeant language server")
    ...
    async def test_main_handles_server_creation_error(self):
        ...
        with patch("pytest_drill_sergeant.lsp.main.logging.basicConfig"):
            with patch("pytest_drill_sergeant.lsp.main.logger") as mock_logger:
                with patch("pytest_drill_sergeant.lsp.main.create_server", side_effect=Exception("Server creation failed")):
                    with patch("pytest_drill_sergeant.lsp.main.sys.exit") as mock_exit:
                        with patch("traceback.print_exc") as mock_traceback:
                            await main()
                            
                            mock_logger.error.assert_called_with("Failed to start language server: Server creation failed")
            ...
            with patch("pytest_drill_sergeant.lsp.main.logger") as mock_logger:
                with patch("pytest_drill_sergeant.lsp.main.create_server") as mock_create:
                    with patch("pytest_drill_sergeant.lsp.main.sys.exit") as mock_exit:
                        with patch("traceback.print_exc") as mock_traceback:
                            mock_server = Mock()
                            mock_server.start_io = AsyncMock(side_effect=Exception("Start failed"))
                            mock_create.return_value = mock_server
                            
                            await main()
                            
                            mock_logger.error.assert_called_with("Failed to start language server: Start failed")
                            mock_traceback.assert_called_once()
                            mock_exit.assert_called_once_with(1)
    ...
    async def test_main_handles_logging_setup_error(self):
        ...
        with patch("pytest_drill_sergeant.lsp.main.logging.basicConfig", side_effect=Exception("Logging failed")):
            with patch("pytest_drill_sergeant.lsp.main.logger") as mock_logger:
                with patch("pytest_drill_sergeant.lsp.main.sys.exit") as mock_exit:
                    with patch("traceback.print_exc") as mock_traceback:
                        await main()
                        
                        mock_logger.error.assert_called_with("Failed to start language server: Logging failed")
                        mock_traceback.assert_called_once()
                        mock_exit.assert_called_once_with(1)
            ...
            with patch("pytest_drill_sergeant.lsp.main.logger") as mock_logger:
                with patch("pytest_drill_sergeant.lsp.main.create_server") as mock_create:
                    with patch("pytest_drill_sergeant.lsp.main.sys.exit") as mock_exit:
                        with patch("traceback.print_exc") as mock_traceback:
                            mock_server = Mock()
                            mock_server.start_io = Mock(side_effect=Exception("Start failed"))
                            mock_create.return_value = mock_server
    ...
    def test_create_server_registers_initialize_feature(self):
        ...
        with patch("pytest_drill_sergeant.lsp.main.DrillSergeantLanguageServer") as mock_server_class:
            with patch("pytest_drill_sergeant.lsp.main.setup_file_watching"):
                ...
                initialize_calls = [call for call in mock_server.feature.call_args_list 
    ...
    def test_create_server_registers_initialized_feature(self):
        ...
        with patch("pytest_drill_sergeant.lsp.main.DrillSergeantLanguageServer") as mock_server_class:
            with patch("pytest_drill_sergeant.lsp.main.setup_file_watching"):
                ...
                initialized_calls = [call for call in mock_server.feature.call_args_list 
...
class TestErrorHandling:
    ...
    def test_create_server_handles_server_instantiation_error(self):
        ...
        with patch("pytest_drill_sergeant.lsp.main.DrillSergeantLanguageServer", side_effect=RuntimeError("Server creation failed")):
    ...
    def test_create_server_handles_feature_registration_error(self):
        ...
        with patch("pytest_drill_sergeant.lsp.main.DrillSergeantLanguageServer") as mock_server_class:
            with patch("pytest_drill_sergeant.lsp.main.setup_file_watching"):
                mock_server = Mock()
                mock_server.feature.side_effect = Exception("Feature registration failed")
                mock_server_class.return_value = mock_server
    ...
    def test_main_handles_import_error(self):
        ...
        with patch("pytest_drill_sergeant.lsp.main.logging.basicConfig"):
            with patch("pytest_drill_sergeant.lsp.main.logger") as mock_logger:
                with patch("pytest_drill_sergeant.lsp.main.create_server", side_effect=ImportError("Module not found")):
                    with patch("pytest_drill_sergeant.lsp.main.sys.exit") as mock_exit:
                        with patch("traceback.print_exc") as mock_traceback:
                            import asyncio
                            asyncio.run(main())
                            
                            mock_logger.error.assert_called_with("Failed to start language server: Module not found")
                            mock_traceback.assert_called_once()
                            mock_exit.assert_called_once_with(1)
...
class TestIntegration:
    ...
    def test_main_and_run_server_consistency(self):
        ...
        with patch("pytest_drill_sergeant.lsp.main.logging.basicConfig") as mock_basic_config_main:
            with patch("pytest_drill_sergeant.lsp.main.logger") as mock_logger_main:
                with patch("pytest_drill_sergeant.lsp.main.create_server") as mock_create_main:
                    # Test main
                    ...
                    # Test run_server
                    with patch("pytest_drill_sergeant.lsp.main.logging.basicConfig") as mock_basic_config_run:
                        with patch("pytest_drill_sergeant.lsp.main.logger") as mock_logger_run:
                            with patch("pytest_drill_sergeant.lsp.main.create_server") as mock_create_run:
                                ...
                                assert mock_basic_config_main.call_args == mock_basic_config_run.call_args
