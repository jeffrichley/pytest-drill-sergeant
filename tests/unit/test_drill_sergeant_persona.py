...
class TestDrillSergeantPersona:
    ...
    def test_test_pass_message(self) -> None:
        ...
        assert any(word in message.upper() for word in [
            "FINALLY", "OUTSTANDING", "GOOD", "EXCELLENT", "PROPER", "ACCEPTABLE", 
            "REAL", "SOLDIER", "BATTLE-READY", "APPROVED", "SATISFACTORY"
        ])
    ...
    def test_test_fail_message_general(self) -> None:
        ...
        assert any(word in message.upper() for word in [
            "DISASTER", "AMATEUR", "UNACCEPTABLE", "PATHETIC", "MESS", "FAILURE", 
            "BARRACKS", "TRAINING", "DISCIPLINE", "RECRUIT"
        ])
        assert len(message) <= persona.config.max_message_length
    ...
    def test_test_fail_message_private_import(self) -> None:
        ...
        assert "test_something" in message
        assert any(word in message.upper() for word in ["PRIVATE", "IMPORT", "SERGEANT", "QUARTERS"])
        assert len(message) <= persona.config.max_message_length
    ...
    def test_test_fail_message_private_attribute(self) -> None:
        ...
        assert "test_something" in message
        assert any(word in message.upper() for word in ["PRIVATE", "ATTRIBUTE", "UNDERWEAR", "SERGEANT"])
        assert len(message) <= persona.config.max_message_length
    ...
    def test_test_fail_message_private_method(self) -> None:
        ...
        assert "test_something" in message
        assert any(word in message.upper() for word in ["PRIVATE", "METHOD", "SERGEANT", "FIRST NAME"])
        assert len(message) <= persona.config.max_message_length
    ...
    def test_summary_excellent(self) -> None:
        ...
        assert any(word in message.upper() for word in [
            "EXCELLENT", "OUTSTANDING", "CHAMPIONSHIP", "BATTLE-READY", "ADEQUATE", "ACCEPTABLE", "REMARKABLY"
        ])
    ...
    def test_summary_good(self) -> None:
        ...
        assert any(word in message.upper() for word in ["ACCEPTABLE", "GOOD", "IMPROVEMENT", "BETTER"])
    ...
    def test_summary_poor(self) -> None:
        ...
        assert any(word in message.upper() for word in [
            "PATHETIC", "UNACCEPTABLE", "DISCIPLINE", "COMBAT", "GROW", "TURN", "NAVIGATE", "PREDICTABLE"
        ])
        # Some templates include the score, others don't - both are valid
        assert len(message) <= persona.config.max_message_length
    ...
    def test_bis_score_excellent(self) -> None:
        ...
        assert any(word in message.upper() for word in ["CHAMPIONSHIP", "SMOOTH", "COMPETENT"])
        assert len(message) <= persona.config.max_message_length
    ...
    def test_bis_score_good(self) -> None:
        ...
        assert "test_something" in message
        assert "75.0" in message
        assert any(word in message.upper() for word in ["GOOD", "IMPROVEMENT", "EXCELLENCE", "AVERAGE"])
        assert len(message) <= persona.config.max_message_length
    ...
    def test_bis_score_poor(self) -> None:
        ...
        assert any(word in message.upper() for word in ["CHAMPION", "IMPROVE", "COURSE", "SUBSTANDARD"])
        assert len(message) <= persona.config.max_message_length
    ...
    def test_brs_score_excellent(self) -> None:
        ...
        assert "90.0" in message
        assert any(word in message.upper() for word in ["CHAMPIONSHIP", "FLEET", "SATISFACTORY"])
        assert len(message) <= persona.config.max_message_length
    ...
    def test_brs_score_poor(self) -> None:
        ...
        assert "45.0" in message
        assert any(word in message.upper() for word in ["CHAMPION", "NAVIGATE", "DISAPPOINTING"])
        assert len(message) <= persona.config.max_message_length
    ...
    def test_get_message_unknown_type(self) -> None:
        ...
        assert "Unknown message type: unknown_type" == message
    ...
    def test_all_templates_have_required_variables(self) -> None:
        ...
        test_cases = [
            ...
            ("test_fail", {"test_name": "test_something", "finding_message": "Some error"}),
            ("test_fail_private_import", {"test_name": "test_something", "finding_message": "Import error"}),
            ("test_fail_private_attribute", {"test_name": "test_something", "finding_message": "Attribute error"}),
            ("test_fail_private_method", {"test_name": "test_something", "finding_message": "Method error"}),
    ...
    def test_message_length_constraints(self) -> None:
        ...
        messages = [
            ...
            persona.on_test_fail("test_something", Finding(
                code="DS201",
            name="private_access",
                severity=Severity.WARNING,
                message="Some violation",
