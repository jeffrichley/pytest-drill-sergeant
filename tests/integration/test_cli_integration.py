...
def _setup_mock_context_and_pipeline(
...
'''
            ...
            try:
                config = LintConfig(
                    paths=[str(test_file_path)], config=str(config_file_path)
                )
                ...
                with (
                    ...
                    mock_context.__exit__ = Mock(return_value=None)
                    mock_context.filter_findings_by_severity.return_value = []
                    mock_context.persona_manager.on_test_pass.return_value = (
                        "Test passed"
                    )
                    mock_context.persona_manager.on_summary.return_value = (
                        "Summary message"
                    )
                    mock_context_class.return_value = mock_context
...
'''
        ...
        try:
            ...
            with (
                ...
                mock_pipeline_instance.analyze_file.side_effect = Exception(
                    "Analysis error"
                )
...
'''
    ...
    def test_sut_filter_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                config = LintConfig(
                    paths=[str(src_file), str(test_file)], sut_filter="src/"
                )
