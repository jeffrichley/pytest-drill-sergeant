...
'''
        ...
        try:
            ...
            assert analysis_time < 3.0, f"CAR analysis took too long: {analysis_time:.2f}s"
            ...
            assert "DS401" in issue_codes, "Should find tests with no assertions (DS401)"
            assert "DS402" in issue_codes, "Should find tests with too many assertions (DS402)"
            ...
            assert len(no_assertion_findings) == 10, f"Expected 10 tests with no assertions, found {len(no_assertion_findings)}"
            assert len(too_many_assertion_findings) == 30, f"Expected 30 tests with too many assertions, found {len(too_many_assertion_findings)}"
    ...
    def test_coverage_signature_performance(self):
        ...
        assert generation_time < 2.0, f"Signature generation took too long: {generation_time:.2f}s"
        ...
        for i in range(min(10, len(signatures))):
            for j in range(i + 1, min(10, len(signatures))):
                similarity = generator.calculate_similarity(signatures[i], signatures[j])
        ...
        assert similarity_time < 1.0, f"Similarity calculation took too long: {similarity_time:.2f}s"
        
        # Should have reasonable similarities
        assert len(similarities) > 0, "Should calculate similarities between signatures"
        assert all(0.0 <= sim <= 1.0 for sim in similarities), "Similarities should be between 0 and 1"
        
        # Verify similarity calculation is working correctly
        # Identical signatures should have similarity of 1.0
        identical_similarity = generator.calculate_similarity(signatures[0], signatures[0])
        assert identical_similarity == 1.0, f"Identical signatures should have similarity 1.0, got {identical_similarity}"
        
        # Different signatures should have similarity < 1.0
        if len(signatures) > 1:
            different_similarity = generator.calculate_similarity(signatures[0], signatures[1])
            assert different_similarity < 1.0, f"Different signatures should have similarity < 1.0, got {different_similarity}"
    ...
    def test_concurrent_coverage_collection(self):
        ...
        try:
            ...
            for i, collector in enumerate(collectors):
                assert hasattr(collector, 'cov'), f"Collector {i} should have coverage object after start"
        ...
        assert total_time < 2.0, f"Concurrent collection took too long: {total_time:.2f}s"
        
        # Verify collectors are in clean state
        for i, collector in enumerate(collectors):
            # Coverage object should still exist but be stopped
            assert hasattr(collector, 'cov'), f"Collector {i} should still have coverage object"
            # Temporary directory should be cleaned up
            assert collector._temp_dir is None, f"Collector {i} should have cleaned up temp directory"
...
'''
        ...
        try:
            ...
            assert analysis_time < 10.0, f"Large file analysis took too long: {analysis_time:.2f}s"
            ...
            test_function_findings = [f for f in findings if f.code in ["DS401", "DS402"]]
            assert len(test_function_findings) > 0, "Should find CAR issues for test functions"
    ...
    def test_coverage_data_serialization_performance(self):
        ...
        assert creation_time < 1.0, f"Coverage data creation took too long: {creation_time:.2f}s"
        ...
        assert access_time < 0.5, f"Coverage data access took too long: {access_time:.2f}s"
