...
'''
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    "-m",
                    "pytest_drill_sergeant.cli.main",
                    "lint",
                    str(test_file),
                    "--profile",
                    "standard",
            ...
            assert (
                "test_with_violations.py" in output_text
            ), "CLI didn't analyze the test file"
...
'''
            ...
            if output_file.exists():
                try:
                    ...
                    assert (
                        "$schema" in sarif_data or "runs" in sarif_data
                    ), "SARIF doesn't have expected structure"
                except json.JSONDecodeError as e:
...
'''
            ...
            output_text = result.stdout + result.stderr
            assert (
                "Files analyzed" in output_text
            ), "CLI didn't report file analysis count"
    ...
    def test_cli_error_handling_smoke(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result.returncode in [
                0,
                1,
            ], f"CLI crashed with non-existent file: {result.returncode}"
    ...
    def test_cli_help_and_commands_smoke(self):
        ...
        # Test main help
        result = subprocess.run(
            [sys.executable, "-m", "pytest_drill_sergeant.cli.main", "--help"],
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        assert (
            "pytest-drill-sergeant" in result.stdout
        ), "Help doesn't mention tool name"
        assert "lint" in result.stdout, "Help doesn't mention lint command"

        # Test lint command help
        result = subprocess.run(
            [sys.executable, "-m", "pytest_drill_sergeant.cli.main", "lint", "--help"],
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        result = subprocess.run(
            [sys.executable, "-m", "pytest_drill_sergeant.cli.main", "demo", "--help"],
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        result = subprocess.run(
            [sys.executable, "-m", "pytest_drill_sergeant.cli.main", "profiles"],
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        result = subprocess.run(
            [sys.executable, "-m", "pytest_drill_sergeant.cli.main", "personas"],
            capture_output=True,
            text=True,
            check=False,
        )
...
'''
            ...
            assert result.returncode in [
                0,
                1,
            ], f"CLI crashed with config file: {result.returncode}"
    ...
    def test_cli_cross_platform_compatibility_smoke(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result.returncode in [
                0,
                1,
            ], f"CLI crashed with Unix line endings: {result.returncode}"
            ...
            assert result.returncode in [
                0,
                1,
            ], f"CLI crashed with Windows line endings: {result.returncode}"
...
'''
            ...
            assert result.returncode in [
                0,
                1,
            ], f"CLI crashed with rich output: {result.returncode}"
            ...
            for persona in personas:
                ...
                assert result.returncode in [
                    0,
                    1,
                ], f"CLI crashed with persona {persona}: {result.returncode}"

                # Should produce output
                output_text = result.stdout + result.stderr
                assert (
                    len(output_text) > 0
                ), f"CLI produced no output with persona {persona}"
