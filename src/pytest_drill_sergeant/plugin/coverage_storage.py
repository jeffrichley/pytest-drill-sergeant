...
class CoverageStorageConfig(BaseModel):
    ...
    storage_path: Path = Field(default=Path(".drill-sergeant/coverage"), description="Base directory for coverage data storage")
    retention_days: int = Field(default=30, ge=0, description="Number of days to retain old data")
    compression: bool = Field(default=True, description="Whether to compress stored data")
...
class SessionData(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=datetime.now, description="When the session was run")
    total_tests: int = Field(ge=0, description="Total number of tests run")
    average_car_score: float = Field(ge=0.0, le=100.0, description="Average CAR score across all tests")
    ...
    config: Dict[str, Any] = Field(default_factory=dict, description="Configuration used for this session")
...
class CoverageTestData(BaseModel):
    ...
    covered_lines: List[int] = Field(default_factory=list, description="Lines covered by this test")
    missing_lines: List[int] = Field(default_factory=list, description="Lines not covered by this test")
    violations: List[str] = Field(default_factory=list, description="Violations found for this test")
    timestamp: datetime = Field(default_factory=datetime.now, description="When this test was run")
...
class FileData(BaseModel):
    ...
    total_coverage: float = Field(ge=0.0, le=100.0, description="Total coverage percentage for this file")
    tests_covering: List[str] = Field(default_factory=list, description="List of test names that cover this file")
    car_scores: List[float] = Field(default_factory=list, description="CAR scores for tests covering this file")
    average_car_score: float = Field(ge=0.0, le=100.0, description="Average CAR score for this file")
...
class CoverageSignatureData(BaseModel):
    ...
    signature_vector: List[float] = Field(default_factory=list, description="Vector representation of the signature")
    signature_pattern: str = Field(description="Pattern representation of the signature")
    timestamp: datetime = Field(default_factory=datetime.now, description="When this signature was created")
...
class SignaturesCollection(BaseModel):
    ...
    signatures: List[CoverageSignatureData] = Field(default_factory=list, description="List of all stored signatures")
...
class CoverageStorage:
    ...
    def __init__(self, config: Optional[CoverageStorageConfig] = None):
    ...
    def _write_json_file(self, file_path: Path, data: BaseModel | Dict[str, Any]) -> None:
    ...
    def _append_to_signatures_file(self, file_path: Path, signature_data: CoverageSignatureData) -> None:
    ...
    def get_test_data(self, test_name: str) -> Optional[CoverageTestData]:
    ...
    def find_similar_signatures(
        self,
        target_signature: CoverageSignature,
        ...
        for signature_data in all_signatures:
            try:
                ...
                similarity_score = self._calculate_similarity(target_signature.signature_hash, signature_data.signature_hash)
        ...
        for session_file in session_files:
            ...
            if data:
                try:
                    ...
                    self.logger.warning(f"Failed to parse session data from {session_file}: {e}")
                    continue
    ...
    def get_coverage_trends(self, days: int = 7) -> Dict[str, Any]:
        ...
        return {
            ...
            "car_score_trend": "improving" if len(car_scores) > 1 and car_scores[-1] > car_scores[0] else "declining",
            "violation_trend": "improving" if len(violation_counts) > 1 and violation_counts[-1] < violation_counts[0] else "declining"
        }


# Global coverage storage instance
_coverage_storage: Optional[CoverageStorage] = None
