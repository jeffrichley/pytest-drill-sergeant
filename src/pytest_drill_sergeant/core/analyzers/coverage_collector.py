...
class CoverageData:
    ...
    def __eq__(self, other) -> bool:
        ...
        return (
            ...
            self.lines_covered == other.lines_covered and
            self.lines_total == other.lines_total and
            self.branches_covered == other.branches_covered and
            self.branches_total == other.branches_total and
            abs(self.coverage_percentage - other.coverage_percentage) < 0.1 and
            self.covered_lines == other.covered_lines and
...
class CoverageCollector:
    ...
    def collect_test_coverage(
        ...
        try:
            ...
            self._analyze_test_coverage(test_file_path, test_name, test_line_number, source_files)
    ...
    def get_coverage_data(self, test_file_path: Path, test_name: str) -> CoverageData | None:
    ...
    def _find_source_files(self, test_file_path: Path) -> list[Path]:
        ...
        try:
            ...
            if self._test_file_contains_source_code(test_file_path):
                ...
                self.logger.debug(f"Test file {test_file_path} contains source code, including it in coverage")
            ...
            for parent in self._safe_parent_traversal(test_dir):
                try:
                    for item in parent.iterdir():
                        if item.is_dir() and not item.name.startswith("test") and item.name != "tests":
                            try:
                                if (item / "__init__.py").exists():
                                    try:
                                        source_files.extend(item.rglob("*.py"))
                                    except (PermissionError, OSError) as e:
                                        self.logger.debug(f"Cannot access package directory {item}: {e}")
                                        continue
                            except (PermissionError, OSError) as e:
                                self.logger.debug(f"Skipping directory {item} due to permission error: {e}")
                                continue
                except (PermissionError, OSError) as e:
                    self.logger.debug(f"Skipping parent directory {parent} due to permission error: {e}")
                    continue
            ...
            self.logger.debug(f"Found {len(source_files)} source files for {test_file_path}")
    ...
    def _safe_parent_traversal(self, start_path: Path) -> list[Path]:
        ...
        system_dirs = {
            "/", "/usr", "/var", "/etc", "/bin", "/sbin", "/lib", "/lib64",
            "/opt", "/home", "/root", "/tmp", "/sys", "/proc", "/dev"
        }
        ...
        try:
            for parent in start_path.parents:
                ...
                if parent_str in system_dirs or parent_str in windows_system_dirs:
                    self.logger.debug(f"Stopping traversal at system directory: {parent_str}")
                    break
                ...
                if len(safe_parents) >= 5:  # Reasonable limit for project structure
                    self.logger.debug(f"Reached maximum traversal depth: {len(safe_parents)}")
                    break
    ...
    def _select_source_files_with_analysis(
        ...
        try:
            ...
            if imported_source_files:
                ...
                for imported_file in imported_source_files:
                    ...
                    if package_dir.exists():
                        for file in package_dir.rglob("*.py"):
                            if (file not in selected_files and 
                                not self._is_test_file(file) and
                                file != imported_file):
                ...
                self.logger.debug(f"Selected {len(selected_files)} files based on import analysis")
            ...
            # If no imported files, use all source files but limit them
                ...
                self.logger.debug(f"Using {len(limited_files)} files (no import analysis available)")
    ...
    def _analyze_test_file_imports(self, test_file_path: Path) -> list[str]:
        ...
        try:
            ...
            imported_modules = [m for m in imported_modules if not self._is_standard_library_module(m)]
            
            self.logger.debug(f"Test file {test_file_path} imports {len(imported_modules)} modules")
    ...
    def _is_standard_library_module(self, module_name: str) -> bool:
        ...
        stdlib_modules = {
            ...
            'itertools', 'functools', 'operator', 'string', 're', 'urllib', 'http',
            'socket', 'threading', 'multiprocessing', 'subprocess', 'logging',
            'unittest', 'pytest', 'pathlib', 'typing', 'dataclasses', 'enum',
            'abc', 'contextlib', 'copy', 'pickle', 'hashlib', 'base64', 'uuid',
            'tempfile', 'shutil', 'glob', 'fnmatch', 'stat', 'fileinput',
            'linecache', 'codecs', 'locale', 'gettext', 'argparse', 'configparser',
            'csv', 'xml', 'html', 'email', 'mimetypes', 'platform', 'ctypes',
            'warnings', 'traceback', 'inspect', 'ast', 'tokenize', 'keyword',
            'builtins', 'io', 'textwrap', 'unicodedata', 'codeop', 'py_compile',
            'compileall', 'dis', 'pickletools', 'distutils', 'setuptools'
        }
    ...
    def _analyze_test_coverage(
        ...
        try:
            ...
            if not test_func:
                self.logger.warning(f"Test function {test_name} not found in {test_file_path}")
                return
        ...
        try:
            ...
            if hasattr(self, 'cov'):
                ...
                if source_file_paths:
                    ...
                    self.logger.debug(f"Coverage source files set to: {source_file_paths}")
    ...
    def _run_test_function(self, test_file_path: Path, test_name: str) -> None:
        ...
        try:
            ...
            if hasattr(test_module, test_name):
                ...
                test_func()
                self.logger.debug(f"Successfully executed standalone test function {test_name}")
            else:
                ...
                for name, obj in vars(test_module).items():
                    if (isinstance(obj, type) and 
                        ...
                        test_method()
                        self.logger.debug(f"Successfully executed class method {test_name} in {name}")
    ...
    def _extract_coverage_data_with_analysis(
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Fallback coverage extraction also failed: {fallback_error}")
    ...
    def _enhance_coverage_with_analysis(
        ...
        try:
            ...
            if analysis_insights:
                enhanced_signature = f"{coverage_data.coverage_signature}|analysis:{analysis_insights}"
                enhanced_data.coverage_signature = enhanced_signature
            
            # Log analysis insights
            self.logger.debug(f"Analysis insights for {coverage_data.test_name}: {analysis_insights}")
    ...
    def _analyze_test_imports(self, test_func: ast.FunctionDef, source_files: list[Path]) -> None:
        ...
        try:
            ...
            for node in ast.walk(test_func):
                if isinstance(node, ast.Import):
                    ...
                    for alias in node.names:
                        ...
                        source_file = self._resolve_import_to_file(module_name, source_files)
                        if source_file:
                    ...
                    # Handle: from module import name
                    if node.module:
                        source_file = self._resolve_import_to_file(node.module, source_files)
                        if source_file:
                            imported_files.add(source_file)
            ...
            self.logger.debug(f"Test function imports {len(imported_files)} source files")
            ...
            self._analysis_results[test_key] = {
                ...
                "coverage_ratio": len(imported_files) / len(source_files) if source_files else 0.0
            }
    ...
    def _analyze_test_calls(self, test_func: ast.FunctionDef, source_files: list[Path]) -> None:
    ...
    def get_analysis_enhanced_coverage_data(self, test_file_path: Path, test_name: str) -> CoverageData | None:
    ...
    def get_coverage_analysis_summary(self, test_file_path: Path, test_name: str) -> dict | None:
    ...
    def _resolve_import_to_file(self, module_name: str, source_files: list[Path]) -> Path | None:
        ...
        try:
            ...
            for source_file in source_files:
                ...
                if source_file.parent.name == module_name and source_file.name == "__init__.py":
                    return source_file
    ...
    def _extract_coverage_data(
        ...
        try:
            ...
            self.logger.debug(f"Coverage data retrieved, measured files: {coverage_data.measured_files()}")
            ...
            for source_file in source_files:
                ...
                file_path_in_measured = str(source_file) in coverage_data.measured_files()
                normalized_path_in_measured = normalized_path in coverage_data.measured_files()
                
                if file_path_in_measured or normalized_path_in_measured:
                    # Use the path that's actually in measured files
                    actual_path = str(source_file) if file_path_in_measured else normalized_path
                    self.logger.debug(f"Source file {source_file} is in measured files (using path: {actual_path})")
                    file_lines = coverage_data.lines(actual_path)
                    file_branches = coverage_data.arcs(actual_path)
                    self.logger.debug(f"File lines: {file_lines}, file branches: {file_branches}")
                    ...
                    if file_lines:
                        ...
                        executed_lines = [line for line in all_executable_lines if line in file_lines]
                        covered_lines += len(executed_lines)
                        self.logger.debug(f"Total lines: {total_lines}, covered lines: {covered_lines}")
                        self.logger.debug(f"All executable lines: {all_executable_lines}")
                        self.logger.debug(f"Executed lines (measured): {executed_lines}")
                    ...
                    self.logger.debug(f"Source file {source_file} is NOT in measured files")
            
            # Calculate coverage percentage
            coverage_percentage = (covered_lines / total_lines * 100) if total_lines > 0 else 0.0
            
            # Generate coverage signature
            coverage_signature = self._generate_coverage_signature(all_covered_lines, source_files)
    ...
    def _generate_coverage_signature(self, covered_lines: set[int], source_files: list[Path]) -> str:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef) and node.name.startswith("test_"):
                    func_findings = self._analyze_test_function_coverage(node, file_path)
                    findings.extend(func_findings)

            self.logger.debug(f"Coverage analysis of {file_path}: {len(findings)} findings")
