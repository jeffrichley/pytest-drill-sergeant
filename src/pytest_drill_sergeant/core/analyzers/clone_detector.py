...
class DynamicCloneDetector:
    ...
    def analyze_test_suite(
        ...
        """
        try:
            self.logger.info(f"Starting dynamic duplicate detection for {len(test_files)} test files")
    ...
    def _generate_test_signature(self, test_func: ast.FunctionDef, test_file: Path) -> str:
        ...
        exception_pattern = self._exception_patterns.get(f"{test_file}:{test_func.name}", "")
    ...
    def _find_duplicate_clusters(self) -> list[DuplicateCluster]:
        ...
        for i, test1_key in enumerate(test_keys):
            ...
            for j, test2_key in enumerate(test_keys[i+1:], i+1):
                ...
                similarity = self._calculate_test_similarity(test1_key, test2_key)
    ...
    def _calculate_test_similarity(self, test1_key: str, test2_key: str) -> float:
        ...
        try:
            ...
            self.logger.error(f"Failed to calculate similarity between {test1_key} and {test2_key}: {e}")
            return 0.0
        ...
        try:
            ...
            for i in range(len(test_keys)):
                for j in range(i + 1, len(test_keys)):
                    sim = self._calculate_test_similarity(test_keys[i], test_keys[j])
            ...
            # Generate cluster ID
            cluster_id = hashlib.md5(f"{representative_test[0]}:{representative_test[1]}".encode()).hexdigest()[:8]
    ...
    def _generate_consolidation_suggestion(self, tests: list[tuple[str, Path]], cluster_type: str) -> str:
        ...
        elif cluster_type == "near_duplicates":
    ...
    def analyze_file(self, file_path: Path) -> list[Finding]:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef) and node.name.startswith("test_"):
                    func_findings = self._analyze_test_function_duplicates(node, file_path)
                    findings.extend(func_findings)
            
            self.logger.debug(f"Duplicate detection analysis of {file_path}: {len(findings)} findings")
