...
class MockOverspecDetector:
    ...
    def _count_mock_assertions(self, func_node: ast.FunctionDef, allowlist: list[str]) -> int:
            ...
            def visit_Call(self, node):
                ...
                if isinstance(node.func, ast.Attribute):
                    if node.func.attr in self.detector.MOCK_ASSERTS:
                        ...
                        if mock_target and self.detector._is_mock_target_allowed(mock_target, self.allowlist):
                            pass
                        else:
                            self.count += 1
