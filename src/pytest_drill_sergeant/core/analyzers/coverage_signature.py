...
class CoverageSignatureGenerator:
    ...
    def _generate_signature_hash(self, coverage_data: CoverageData) -> str:
        ...
        try:
            ...
            if coverage_data.covered_lines:
                lines_str = ",".join(sorted(str(line) for line in coverage_data.covered_lines))
    ...
    def _generate_signature_vector(self, coverage_data: CoverageData) -> list[float]:
        ...
        try:
            ...
            vector.append(coverage_data.lines_covered / max(coverage_data.lines_total, 1))
            vector.append(coverage_data.branches_covered / max(coverage_data.branches_total, 1))
            ...
            if coverage_data.covered_lines:
                ...
                if len(lines) > 1:
                    ...
                    vector.append(1.0 / (1.0 + avg_gap))  # Higher density = higher value
                else:
                    vector.append(1.0)
            else:
                vector.append(0.0)
    ...
    def _generate_coverage_pattern(self, coverage_data: CoverageData) -> str:
        ...
        try:
            ...
            # Add line coverage
            pattern_parts.append(f"lines:{coverage_data.lines_covered}/{coverage_data.lines_total}")
            
            # Add branch coverage
            pattern_parts.append(f"branches:{coverage_data.branches_covered}/{coverage_data.branches_total}")
    ...
    def calculate_similarity(
        self, 
        signature1: CoverageSignature, 
        signature2: CoverageSignature
    ) -> float:
        ...
        try:
            ...
            if signature1.signature_vector and signature2.signature_vector:
                similarity = self._cosine_similarity(
                    signature1.signature_vector, 
                    signature2.signature_vector
                )
                return similarity
            
            # Fallback to pattern comparison
            return self._pattern_similarity(signature1.coverage_pattern, signature2.coverage_pattern)
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef) and node.name.startswith("test_"):
                    func_findings = self._analyze_test_function_signature(node, file_path)
                    findings.extend(func_findings)

            self.logger.debug(f"Coverage signature analysis of {file_path}: {len(findings)} findings")
