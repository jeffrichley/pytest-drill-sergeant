...
class AAACommentDetector:
    ...
    def _analyze_test_function(
        self, func_node: ast.FunctionDef, comments: dict[int, str], file_path: Path
    ) -> list[Finding]:
    ...
    def _analyze_aaa_structure(
        self, func_node: ast.FunctionDef, comments: dict[int, str]
    ) -> AAAResult:
        """Analyze AAA structure of a test function.
    ...
    def _extract_aaa_sections_from_comment(self, comment: str) -> list[str]:
        ...
        # Handle comma-separated sections (e.g., "arrange, act, assert")
        if "," in comment:
            parts = [part.strip() for part in comment.split(",")]
            for part in parts:
                sections_found.extend(self._extract_sections_from_part(part))
        else:
            # Handle single or mixed sections (e.g., "arrange data and act on it")
            sections_found.extend(self._extract_sections_from_part(comment))
