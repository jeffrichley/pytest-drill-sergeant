...
class RuleConfig(BaseModel):
    ...
    severity: SeverityLevel | None = Field(None, description="Override severity level")
    threshold: float | None = Field(
        None, ge=0.0, le=1.0, description="Rule-specific threshold"
    )
    tags: list[str] = Field(default_factory=list, description="Rule-specific tags")
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="Rule-specific metadata"
    )
...
class ProfileConfig(BaseModel):
    ...
    rules: dict[str, RuleConfig] = Field(
        default_factory=dict, description="Rule configurations"
    )
    thresholds: dict[str, float] = Field(
        default_factory=dict, description="Profile thresholds"
    )
    ...
    def validate_rule_codes(cls, v: dict[str, RuleConfig]) -> dict[str, RuleConfig]:
        ...
        for rule_code in v:
            if not RuleRegistry.is_valid_rule(rule_code):
                ...
                raise ValueError(
                    f"Invalid rule code '{rule_code}'. Use 'DS###' format."
                )
        return v
...
class OutputConfig(BaseModel):
    ...
    json_path: Path | None = Field(None, description="Path for JSON output")
    sarif_path: Path | None = Field(None, description="Path for SARIF output")
...
class AnalysisConfig(BaseModel):
    ...
    timeout: float | None = Field(
        None, ge=1.0, description="Analysis timeout in seconds"
    )
    memory_limit: int | None = Field(None, ge=100, description="Memory limit in MB")
...
class DSConfig(BaseModel):
    ...
    schema_version: SchemaVersion = Field(
        SchemaVersion.V1_0, description="Configuration schema version"
    )

    # Core settings
    profile: Profile = Field(
        Profile.STANDARD, description="Active configuration profile"
    )
    profiles: dict[str, ProfileConfig] = Field(
        default_factory=dict, description="Available profiles"
    )
    ...
    rules: dict[str, RuleConfig] = Field(
        default_factory=dict, description="Global rule configurations"
    )
    ...
    ignore_patterns: list[str] = Field(
        default_factory=list, description="Files/patterns to ignore"
    )

    # Per-file ignores
    per_file_ignores: dict[str, list[str]] = Field(
        default_factory=dict, description="Rule ignores per file pattern"
    )
    ...
    per_file_severity_overrides: dict[str, SeverityLevel] = Field(
        default_factory=dict, description="Severity overrides per file pattern"
    )
    ...
    @field_validator(
        "test_patterns", "ignore_patterns", "include_patterns", "exclude_patterns"
    )
    @classmethod
    ...
    @classmethod
    def validate_per_file_severity_overrides(
        cls, v: dict[str, SeverityLevel]
    ...
    @classmethod
    def validate_per_file_rule_overrides(
        cls, v: dict[str, dict[str, RuleConfig]]
    ) -> dict[str, dict[str, RuleConfig]]:
...
def create_default_config() -> DSConfig:
    ...
    for rule_code, rule_config in strict_rules.items():
        rule_config.severity = (
            SeverityLevel.ERROR
            if rule_config.severity == SeverityLevel.WARNING
